...
for d in ['../utilities']:
    module_path = os.path.abspath(d)
    if module_path not in sys.path:
        sys.path.append(module_path)

from remove_background_functions import remove_bg, replace_text

def textural_features_generator(df_features, path_name, path_img):
    filename=path_name+'/'+ path_img
    idx_name = replace_text(path_img, ['/', ' (', ').JPG'], rep='_')
    bgr_img  = cv.resize(pcv.readimage(filename)[0], (256,256), interpolation=cv.INTER_CUBIC)
    f = cv.resize(pcv.readimage(filename, mode='gray')[0], (256,256), interpolation=cv.INTER_CUBIC)
    ...
    # First Order Statistics (FOS) are calculated from the histogram of the image which is the empirical probability density function for single pixels. The FOS features are the following: 
    # 1) mean, 
    ...
    df_features.loc[idx_name, labels] = list(features)
    ...
    features_mean, features_range, labels_mean, labels_range = pf.glcm_features(f, ignore_zeros=True)
    features, labels = np.array([features_mean,features_range]).flatten(), np.array([labels_mean, labels_range]).flatten()
    df_features.loc[idx_name, labels] = list(features)
    ...
    features, labels = pf.glds_features(f, mask, Dx=[0,1,1,1], Dy=[1,1,0,-1])
    df_features.loc[idx_name, labels] = list(features)
    ...
    features, labels = pf.shape_parameters(f, mask, perimeter, pixels_per_mm2=1)
    df_features.loc[idx_name, labels] = list(features)
    ...
    df_features.loc[idx_name, labels] = list(np.array(features).flatten())
    ...
    features = pf.multilevel_binary_morphology_features(f, mask, N=N, thresholds=[25, 50])
    labels = [f'{f}{i}' for f in ['pdf_L', 'pdf_M', 'pdf_H', 'cdf_L', 'cdf_M', 'cdf_H'] for i in range(N)]
    df_features.loc[idx_name, labels] = list(np.array(features).flatten())
    ...
    features, labels = pf.multiregion_histogram(f, mask, bins, num_eros=3, square_size=3)
    df_features.loc[idx_name, labels] = list(features)
    ...
    dig, hist = 8, 8
    Hd, Ht, labels = pf.correlogram(f, mask, bins_digitize=dig, bins_hist=hist, flatten=True)
    labels = [f'Correlogram_{c}_{i}_{j}' for c in ['dist','angl'] for i in range(dig) for j in range(hist)]
    df_features.loc[idx_name, labels] = list(np.array([Hd,Ht]).flatten())
