...
def mask_from_lab(img, k=2, ch='S'):
    ...
    lab[:,:,ch] = np.where(center[label.flatten()].reshape(lab[:,:,ch].shape) == list(sorted(set(center.flatten())))[-1], 0, 255*np.ones(lab[:,:,ch].shape))
...
def houghLines(img_in):
    ...
    minLineLength=0
    lines = cv.HoughLinesP(image=edges,rho=0.02,theta=np.pi/256, threshold=15,lines=np.array([]), minLineLength=minLineLength,maxLineGap=21)
...
def hog_process(img):
    ...
    img1=np.array(cv.GaussianBlur(np.array(img[:,:128,:]), (9,9), cv.BORDER_DEFAULT), dtype='uint8')
    img2=np.array(cv.GaussianBlur(np.array(img[:,128:,:]), (9,9), cv.BORDER_DEFAULT), dtype='uint8')

    _, hog_img1 = hog(img1, orientations=9, pixels_per_cell=(8,8),cells_per_block=(2, 2), visualize=True, multichannel=True)
    _, hog_img2 = hog(img2, orientations=9, pixels_per_cell=(8,8),cells_per_block=(2, 2), visualize=True, multichannel=True)
...
def thresh_morpho_lab(img):
    ...
    l,a,b = cv.split(cv.pyrMeanShiftFiltering(cv.cvtColor(img, cv.COLOR_BGR2LAB), 0, 255))
...
def remove_bg(img):
    ...
    for k in color_dict_HSV.keys():
        HSV[k], _ = pcv.threshold.custom_range(img=blur, lower_thresh=color_dict_HSV[k][1], upper_thresh=color_dict_HSV[k][0], channel='HSV')
        
        # Get countours and fill inside contours
        cnts = cv.findContours(HSV[k], cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE | cv.RETR_EXTERNAL)
...
def preprocess_images(file_name):
    try:
        bgr_img  = cv.resize(pcv.readimage(file_name)[0], (256,256), interpolation=cv.INTER_CUBIC)
        gray_img = cv.resize(pcv.readimage(file_name, mode='gray')[0], (256,256), interpolation=cv.INTER_CUBIC)
        ...
        normalized_img = cv.normalize(np.array(enhanced), None, alpha=0, beta=1, norm_type=cv.NORM_MINMAX, dtype=cv.CV_32F)
        ...
        dilated = np.array(cv.dilate(canny, (5,5), iterations=5), dtype='uint8')
        eroded  = np.array(cv.erode(dilated, (5,5), iterations=5), dtype='uint8')

        enh_canny = cv.bitwise_and(np.array(enhanced, dtype='uint8'),np.array(enhanced, dtype='uint8'),mask=np.invert(canny))
        crop_res  = cropp_resize_image(canny, enh_canny)
