'''
This file contains function to extract features from image.
...
def get_pyfeats_features(df_features, specie_index, raw_bgr_img, mask):
    # raw_bgr_img  = cv.resize(pcv.readimage(filename)[0], (256,256), interpolation=cv.INTER_CUBIC)
    # f = cv.resize(pcv.readimage(filename, mode='gray')[0], (256,256), interpolation=cv.INTER_CUBIC)

    # mask, _ = remove_bg(raw_bgr_img)
    mask = mask // 255
    ...
    df_features.loc[specie_index, labels] = list(features)
    ...
    df_features.loc[specie_index, labels] = list(np.array(features).flatten())
    ...
    df_features.loc[specie_index, labels] = list(np.array([Hd, Ht]).flatten())
...
def get_lbp_histogram(df_features, specie_index, img):
    radius = 4
    n_points = 4 * radius
    ...
    hist /= (hist.sum() + 1e-7)
    df_features[specie_index, 'lbp_histogram'] = hist
    return df_features


def get_hue_moment(df_features, specie_index, image):
    '''
    Calculate the Hu Moments of an image.
    '''

    image = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    df_features[specie_index, 'hue_moment'] = cv.HuMoments(
        cv.moments(image)).flatten()
    return feature


def get_haralick(df_features, specie_index, image):
    '''
    Calculate the Haralick texture features of an image.
    '''

    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    df_features[specie_index, 'haralick'] = mahotas.features.haralick(
        gray).mean(axis=0)
    return df_features


def get_hsv_histogram(df_features, specie_index, image, mask=None):
    '''
    ...
    df_features[specie_index, 'hsv_histogram'] = hist.flatten()
    return df_features


def get_lab_histogram(df_features, specie_index, image, mask=None):
    '''
    ...
    hist = cv.calcHist([image], [0, 1, 2], None, [
                       8, 8, 8], [0, 256, 0, 256, 0, 256])
    cv.normalize(hist, hist)
    df_features[specie_index, 'lab_histogram'] = hist.flatten()
    return df_features


def get_graycoprops(df_features, specie_index, img):
    ...
    glcm = feature.graycomatrix(img, [1, 2, 3], angles=[
    ...
    df_features[specie_index, 'graycoprops'] = features.flatten()
    return df_features
